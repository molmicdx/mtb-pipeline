...
singularity exec -B $MNT_SINGULARITY:/mnt $SINGULARITY/$CUTADAPT cutadapt --cores 8 -q 5 -b file:/mnt/NGSTYP/pipeline/Illumina_adaptors_v2.fa -B file:/mnt/NGSTYP/pipeline/Illumina_adaptors_v2.fa --minimum-length=20 -o /mnt/mtb_amr/$READS_DIR/trimmed/$1.R1.trimmed.fq.gz -p /mnt/mtb_amr/$READS_DIR/trimmed/$1.R2.trimmed.fq.gz /mnt/mtb_amr/$READS_DIR/$1_R1.fq.gz /mnt/mtb_amr/$READS_DIR/$1_R2.fq.gz > $READS_DIR/trimmed/$1_cutadapt.log 2>&1
echo "Done"

# 6. Map reads
echo "[bwa mem] Mapping reads to reference genome..."
singularity exec -B $MNT_SINGULARITY:/mnt $SINGULARITY/$BWA bwa mem /mnt/mtb_amr/$REFERENCE_GENOME /mnt/mtb_amr/$READS_DIR/trimmed/$1.R1.trimmed.fq.gz /mnt/mtb_amr/$READS_DIR/trimmed/$1.R2.trimmed.fq.gz -K 100000000 -R '@RG\tID:'$1'\tLB:LB_'$1'\tPL:illumina\tPU:TEST_RUN\tSM:NC_000962.3' > $MAPPED_DIR/$1_trimmedRG.sam
echo "Done"
