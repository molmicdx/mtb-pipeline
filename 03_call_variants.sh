...
singularity exec -B $PWD $SINGULARITY/$GATK gatk HaplotypeCaller -ploidy 1 -R $REFERENCE_GENOME -I $DEDUPED_DIR/$1_deduped_mq10.bam -O $VC_DIR/gvcf/$1_mq10_gatk_no_gt.g.vcf -ERC GVCF > $VC_DIR/$1_mq10_gatk.log 2>&1
echo "[GATK GenotypeGVCFs] Genotyping variants..."
singularity exec -B $PWD $SINGULARITY/$GATK gatk GenotypeGVCFs -R $REFERENCE_GENOME -V $VC_DIR/gvcf/$1_mq10_gatk_no_gt.g.vcf -O $VC_DIR/$1_mq10_gatk.vcf >> $VC_DIR/$1_mq10_gatk.log 2>&1
echo "Done"
...
docker run -v `pwd`:`pwd` -w `pwd` -i -t --rm $FREEBAYES freebayes -f $REFERENCE_GENOME -p 1 --min-alternate-fraction 0.2 --gvcf $DEDUPED_DIR/$1_deduped_mq10.bam > $VC_DIR/gvcf/$1_mq10_freebayes.g.vcf
echo "Done"
...
singularity exec -B $PWD $SINGULARITY/$GATK gatk LeftAlignAndTrimVariants -R $REFERENCE_GENOME -V $VC_DIR/gvcf/$1_mq10_freebayes.g.vcf -O $VC_DIR/gvcf/$1_mq10_freebayes_normalized.g.vcf >> $VC_DIR/$1_mq10_freebayes_normalized.log 2>&1
echo "Done"
...
singularity exec -B $PWD $SINGULARITY/$GATK gatk LeftAlignAndTrimVariants -R $REFERENCE_GENOME -V $VC_DIR/gvcf/$1_mq10_deepvariant.g.vcf -O $VC_DIR/gvcf/$1_mq10_deepvariant_normalized.g.vcf >> $VC_DIR/deepvariant/$1_mq10_deepvariant_normalized.log 2>&1
echo "Done"
